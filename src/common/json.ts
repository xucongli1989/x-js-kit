...
/**
   ...
   * @returns 判断结果
   */
export const hasKey = (json: AnyKeyValueType, keyName: string) => {
    if (!json) {
        return false
    }
 ...
 * @returns 判断结果
 */
export const hasValue = (json: AnyKeyValueType, keyValue: any) => {
    if (!json) {
        return false
    }
    let r = false
 ...
 * @returns 转换结果
 */
export const toParams = (json: AnyKeyValueType) => {
    if (!json) return "";
    let arr = [], temp = "";
    for (let m in json) {
